...
| 12 | TFLite / on-device ML | On-device TensorFlow.js MobileNet V1 (alpha 0.25) with ~1.92 MB weights bundled in the app (`public/models/mobilenet/`); inference runs entirely in the WebView with no CDN fallback, and "Suggest category" works from the selected local file before the upload completes. ⚠️ Physical-device **offline** proof is pending (10L blocked: no adb/device) — implementation + unit tests only | mobile `services/ml.ts`, `public/models/mobilenet/*`; create-report "Suggest category" | attach photo → suggest category before publishing | `ml.test.ts`, category-mapping unit tests (ml map in shared) | DONE |
...
| 24 | Task-specific deliverable | Android debug APK **builds** (`app-debug.apk`, ~6.8 MB, Supabase-only, no secret/localhost/port-4000) and the PWA web shell runs; the physical install/launch walkthrough is **not yet observed** — 10L is blocked (no adb / no attached authorized device) | `apps/mobile/android/`, `npm run apk` | install `app-debug.apk` on a physical device and run the walkthrough | APK build; physical run pending | NOT_DONE |
