...
| **ARM64 phone pass — on-device VLM GPU delegate** | **BLOCKED** — no authorized physical ARM64 device attached; `adb` not available. Real 500M GPU generation, 256M diagnostic outcome, and photo-picker → VLM walkthrough cannot be verified on host `x86_64`. | Provide authorized ARM64 phone + `adb`; run 10L | `apps/mobile/src/services/vlm.ts`, `apps/mobile/android/` (GPU delegate build), `docs/FINAL_ACCEPTANCE_REPORT.md` | **No — #23 GPU proof and #24 deliverable depend on this** |
| **ARM64 phone pass — on-device text embeddings (Possible Matches)** | **BLOCKED** — same as above. Offline cosine top-3 proof requires physical device. | Provide authorized ARM64 phone + `adb`; run 10L | `apps/mobile/src/services/ml.ts`, `docs/FINAL_ACCEPTANCE_REPORT.md` | **No — #12 device proof depends on this** |
